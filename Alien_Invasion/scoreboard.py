...
#A class to report scoring information
	...
	def prep_score(self):
		...
		#Render the ships
		#render_text(("Ships Left: " + str(self.stats.ships_left)), self.screen, self.screen_rect.right - 20, 35)
	...
	def show_score(self, state):
		if state != "dead":
			...
			self.shipsleft_image = self.font.render(str("Ships Left: " + str(self.stats.ships_left + 1)), True, self.text_color, self.ai_settings.bg_color)
			self.shipsleft_rect = self.shipsleft_image.get_rect()
			self.screen.blit(self.shipsleft_image, self.shipsleft_rect)
...
def render_text(text, screen, x, y):
	#font = pygame.font.Font("fonts/VT323-Regular.ttf", 48)
